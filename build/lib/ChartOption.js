...
const GAUGE_POINTER = 'path://M2.9,0.7L2.9,0.7c1.4,0,2.6,1.2,2.6,2.6v115c0,1.4-1.2,2.6-2.6,2.6l0,0c-1.4,0-2.6-1.2-2.6-2.6V3.3C0.3,1.9,1.4,0.7,2.9,0.7z';
    ...
    */
    ...
    getSumSeries(data, stacked) {
        ...
        return (Object.keys(stacks)
            ...
            const length = lines.reduce((max, i) => Math.max(max, data[i]?.length || 0), 0);
            const sums = [];
            ...
            return {
                ...
                label: {
                    ...
                    formatter: (value) => sums[value.dataIndex] === null
                        ? ''
    ...
    getYAxis(series) {
        return this.config.l.map((oneLine, chartIndex) => {
            ...
            if (yAxis[chartIndex]) {
                const dataMin = yAxis[chartIndex].min;
                const dataMax = yAxis[chartIndex].max;
    ...
    renderTooltip(params) {
        ...
        const shiftFormat = this.config.timeFormat || (this.chart.withSeconds ? 'L HH:mm:ss' : this.chart.withTime ? 'L HH:mm' : 'L');
     ...
     */
    buildGauge(option, actualValues) {
        const circles = this.config.gaugeShape !== 'gauge';
        const fontSize = parseInt(this.config.barFontSize, 10) || 14;
        const fontColor = this.config.barFontColor || (this.themeType === 'light' ? '#000' : '#FFF');
        ...
        const longest = values.reduce((max, value, index) => Math.max(max, this.yFormatter(value, index, true).length), 0);
        ...
        const data = this.config.l.map((oneLine, index) => {
            ...
            return {
                ...
                title: {
                    ...
                    overflow: circles ? undefined : 'truncate',
                },
                detail: {
                    offsetCenter: circles ? [0, step * row] : [step * slot, '97%'],
                    formatter: (val) => this.yFormatter(shared ? val : values[index], index, true),
                },
            };
        });
        option.series = [
    ...
    getOption(data, config, actualValues, categories) {
        ...
        if (this.isGauge()) {
            ...
            option.series.forEach((item, chartIndex) => {
                ...
                const max = oneLine.max ? parseFloat(oneLine.max) || undefined : undefined;
            ...
            if (!this.compact && !this.config.autoGridPadding) {
                ...
                option.grid.right = padRight + 10 + (this.hasRightToolbar() ? 20 : 0);
