...
class ChartModel {
    ...
    processRawData(
        ...
        for (let i = 0; i < values.length; i++) {
            ...
            if ((values[i].val as unknown as string) === 'true' || (values[i].val as unknown as boolean) === true) {
                values[i].val = 1;
            } else if (
                (values[i].val as unknown as string) === 'false' ||
                (values[i].val as unknown as boolean) === false
            ) {
                values[i].val = 0;
            } else if (typeof values[i].val === 'string') {
                values[i].val = parseFloat(values[i].val as unknown as string);
            }
            ...
            if (line.chartType === 'bar') {
                ...
                for (let c = 0; c < barCategories.length; c++) {
                    if (
                        ...
                        if (convertFunc) {
                            _barSeries[c].push(
                                values[i].val !== null ? convertFunc((values[i].val as number) + yOffset) : null,
                            );
                        } else {
                            _barSeries[c].push(values[i].val !== null ? (values[i].val as number) + yOffset : null);
                        }
                        break;
                    }
                }
                ...
                let val;
                if (convertFunc) {
                    val = values[i].val !== null ? convertFunc((values[i].val as number) + yOffset) : null;
                } else {
                    val = values[i].val !== null ? (values[i].val as number) + yOffset : null;
                }
                const dp: EchartOneValue = { value: [values[i].ts, val] };
    ...
    async readOneChart(id: string, instance: string, index: number): Promise<void> {
        if (instance === 'json') {
            ...
            try {
                ...
                const result = this.processRawData(id, this.config.l[index], values);
                if (result.barData) {
            ...
            if (
                (this.config.legActual &&
                    this.config.l[index].chartType !== 'bar' &&
                    this.config.l[index].chartType !== 'polar') ||
                this.config.l[index].aggregate === 'current'
            ) {
                // read current value
                try {
                    const state = await this.socket.getState(id);
                    this.actualValues[index] =
                        state && (state.val || state.val === 0 || state.val === false) ? (state.val as number) : null;
                } catch (e) {
    ...
    _readObject(id: string): Promise<ioBroker.Object | null> {
        if (!(this.objectPromises[id] instanceof Promise)) {
            this.objectPromises[id] = this.socket.getObject(id).catch(e => {
                if (e === NOT_CONNECTED && this.onErrorFunc) {
                    this.onErrorFunc(e);
                }
    ...
    async _readOneLine(index: number): Promise<void> {
        try {
            ...
            if (obj?.common) {
                const name: ioBroker.StringOrTranslated = this.config.l[index].name || obj.common.name;

                this.config.l[index].name =
                    name && typeof name === 'object'
                        ? name[this.systemConfig.language] || name.en || this.config.l[index].id
                        : (name as string) || '';

                this.config.l[index].unit =
                    this.config.l[index].unit || (obj.common.unit ? obj.common.unit.replace('�', '°') : '');
                ...
                if (this.config.l[index].chartType === 'auto') {
                    ...
                    this.config.l[index].aggregate = obj.common.type === 'boolean' ? 'none' : 'minmax';
                }
                    ...
                    if (Object.keys(this.config.l[index].states).find(key => key === 'true' || key === 'false')) {
                        ...
                        Object.keys(this.config.l[index].states).forEach(key => {
                            states[key === 'true' ? 1 : key === 'false' ? 0 : key] = (
                                this.config.l[index].states as Record<string, string>
                            )[key];
                        });
    ...
    onStateChange = (id: string, state: ioBroker.State | null | undefined): void => {
        ...
        for (let index = 0; index < this.config.l.length; index++) {
            if (this.config.l[index].id === id) {
                ...
                if (this.actualValues && this.actualValues[index] !== state.val) {
                    this.actualValues[index] = state.val;
                    changed = true;
                }
                break;
            }
