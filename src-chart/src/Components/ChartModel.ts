...
class ChartModel {
    ...
    private isCurrentValueOnly(lineConfig: ChartLineConfig): boolean {
        return (
            this.config.chartMode === 'donut' || this.config.chartMode === 'gauge' || lineConfig.aggregate === 'current'
        );
    }
    ...
    hasBarDiff(): boolean {
        return (
            this.config.postProcessing === 'diff' ||
     ...
     */
    private extendZoomLimitToData(): void {
     ...
     */
    private rememberZoomLimit(referenceEnd: number): void {
        if (!this.zoomData) {
     ...
     */
    static smoothSeries(seriesData: LineSeries, count: number | undefined): void {
        const window = Math.round(getFloat(count));
        if (window < 2 || seriesData.length < 2) {
            return;
        }

        const values: (number | null)[] = seriesData.map(item => item.value[1]);
