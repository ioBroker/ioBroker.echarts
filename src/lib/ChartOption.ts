...
type ChartInfo = {
    ...
    /**
     * Distance from the border of the canvas for a title or a legend that stands outside the chart.
     * Filled by `reserveOutsideBands`, which also takes the place they need away from the grid.
     */
    outsideTitleTop?: number;
    outsideTitleBottom?: number;
    outsideLegendTop?: number;
    outsideLegendBottom?: number;
...
class ChartOption {
    ...
    */
        ...
        return this.config.l.map((oneLine, chartIndex) => {
            ...
            return {
                ...
                axisLabel: {
                    ...
                    formatter: (value: number): string => this.yFormatter(value, chartIndex, true, false, false, true),
                    color:
     ...
     */
    getAfterComma(line: number, lineDigitsOnly?: boolean): number | undefined {
    ...
    getLegend(actualValues: number[]): LegendComponentOption {
        ...
        const legend: LegendComponentOption = {
            ...
            left:
                this.config.legend === 'n' || this.config.legend === 's'
                    ? 'center'
    ...
    getTitleSide(): 'top' | 'bottom' | undefined {
     ...
     */
    reserveOutsideBands(option: EChartsOption): void {
        ...
        const titleHeight = (parseInt(this.config.titleSize as unknown as string, 10) || 20) + 8;
        ...
        const legendHeight =
            parseInt(this.config.legendHeight as unknown as string, 10) ||
            Math.max(parseInt(this.config.legFontSize as unknown as string, 10) || 12, 14) + 10;
        ...
        grid.top = ((grid.top as number) || 0) + top;
        grid.bottom = ((grid.bottom as number) || 0) + bottom;
     ...
     */
    buildDonut(option: EChartsOption, actualValues: number[]): void {
        ...
        const data: { name: string; value: number; lineIndex: number; itemStyle: { color?: string } }[] =
            this.config.l.map((oneLine, index) => {
        ...
        const lineIndexOf = (params: CallbackDataParams): number =>
            (params.data as { lineIndex?: number })?.lineIndex ?? 0;

        const fontSize = parseInt(this.config.barFontSize as unknown as string, 10) || 12;
        const fontColor = this.config.barFontColor || (this.themeType === 'light' ? '#000' : '#FFF');

        let formatter: ((params: CallbackDataParams) => string) | undefined;
        switch (labels) {
            case 'name':
                formatter = (params): string => params.name;
                break;
            case 'value':
                formatter = (params): string => this.yFormatter(params.value as number, lineIndexOf(params), true);
                break;
            case 'percent':
                formatter = (params): string => `${params.percent} %`;
                break;
            case 'nameValue':
                formatter = (params): string =>
                    `${params.name}: ${this.yFormatter(params.value as number, lineIndexOf(params), true)}`;
                break;
            case 'namePercent':
                formatter = (params): string => `${params.name}: ${params.percent} %`;
                break;
            default:
                formatter = undefined;
        }
        ...
        option.tooltip = {
            ...
            formatter: (params: CallbackDataParams): string =>
                `${params.marker as string}${params.name}: ${this.yFormatter(
