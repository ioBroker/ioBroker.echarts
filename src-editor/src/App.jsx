...
import { I18n, Utils, Loader, withWidth, GenericApp } from '@iobroker/adapter-react-v5';
...
function loadChartParam(name, defaultValue) {
    return window.localStorage.getItem(`App.echarts.__${name}`)
        ? window.localStorage.getItem(`App.echarts.__${name}`)
        : defaultValue;
}
...
class App extends GenericApp {
    ...
    onHashChanged() {
        ...
        if (
            (config.preset && this.state.selectedId !== config.preset) ||
            (config.id &&
                (this.state.selectedId?.id !== config.id || this.state.selectedId?.instance !== config.instance))
        ) {
    ...
    onConnectionReady() {
        ...
        this.socket
            ...
            .then(instances =>
                instances.filter(entry => entry && entry.common && entry.common.getHistory && entry.common.enabled),
            )
    ...
    getNewPresetName(parentId, prefix, index) {
        ...
        return this.socket
            .getObject(`${this.adapterName}.${this.instance}.${parentId ? `${parentId}.` : ''}${prefix}${index}`)
            .then(obj => {
                if (!obj) {
                    return prefix + index;
                }
    ...
    onCreatePreset = (isFromCurrentSelection, parentId) => {
        if (isFromCurrentSelection === true) {
            ...
            if (
                !chartsList.find(
                    item => item.id === this.state.selectedId.id && item.instance === this.state.selectedId.instance,
                )
            ) {
            ...
            return new Promise(resolve => {
                if (chartsList.length === 1) {
                    this.socket.getObject(chartsList[0].id).then(obj => resolve(obj));
                    return;
                }
                resolve(null);
            })
                .then(obj => {
                    if (obj) {
                        name = (
                            obj && obj.common && obj.common.name
                                ? Utils.getObjectNameFromObj(obj, null, { language: I18n.getLanguage() })
                                : ''
                        ).trim();
                    }

                    return this.getNewPresetName(parentId, name).then(_name => {
                        const template = {
                            common: {
                                name: _name,
                                expert: true,
    ...
    savePreset = () => {
        ...
        return this.socket
            .getObject(this.state.selectedId)
            .then(obj => {
                if (!obj || !obj.native) {
    ...
    loadChartOrPreset(selectedId, cb) {
        ...
        if (selectedId && typeof selectedId === 'object') {
            ...
            if (this.state.chartsList) {
                this.state.chartsList.forEach(
                    item => !this.objects[item.id] && promises.push(this.socket.getObject(item.id)),
                );
                !this.objects[selectedId.id] &&
                    !this.state.chartsList.find(item => item.id === selectedId.id) &&
            ...
            Promise.all(promises)
                .then(results => {
                    ...
                    const lines = (this.state.chartsList || []).map(item =>
                        DefaultPreset.getDefaultLine(
                    ...
                    (!this.state.chartsList ||
                        !this.state.chartsList.find(
                            item => item.id === selectedId.id && item.instance === selectedId.instance,
                        )) &&
                        lines.push(
                            DefaultPreset.getDefaultLine(
                                this.state.systemConfig,
                        ...
                        <SettingsEditor
                            ...
                            onAutoSave={autoSave => {
                                ...
                                if (autoSave && this.state.selectedPresetChanged) {
                                    this.savePreset().then(() => this.setState({ autoSave }));
                                } else {
        ...
        if (destination && draggableId.includes('***') && source.droppableId === 'Lines') {
            ...
            this.socket
                ...
                .then(obj => {
                    ...
                    const newLine = DefaultPreset.getDefaultLine(
                        this.state.systemConfig,
                        instance,
                        obj,
                        I18n.getLanguage(),
                    ...
                    for (let i = 0; i < presetData.lines.length; i++) {
                        ...
                        if (presetData.lines[i].commonYAxis >= destination.index.toString()) {
                            presetData.lines[i].commonYAxis = (
                                parseInt(presetData.lines[i].commonYAxis, 10) + 1
                            ).toString();
                        }
                    }
